...
export default connect((state) => {
  ...
  /* YOUR CODE GOES HERE */
  const filteredAccounts = utils.filterAccounts(
    state.accounts,
    state.userInput
  );

  const filteredJournal = utils
    .filterJournal(state.journalEntries, state.userInput)
    .filter((entry) =>
      filteredAccounts.find((a) => a.ACCOUNT === entry.ACCOUNT)
    );

  balance = utils.calculateBalance(filteredJournal);

  balance.forEach((entry) => {
    const account = filteredAccounts.find((a) => a.ACCOUNT === entry.ACCOUNT);

    entry["DESCRIPTION"] = account.LABEL;
  });
